...
def test_pii_scrub_phone() -> None:
    s = Scrubber()
    out = s.scrub("我的手机13900000001收不到验证码")
...
def test_pii_scrub_phone_stable_mapping() -> None:
    s = Scrubber()
    out1 = s.scrub("13900000001 first")
    out2 = s.scrub("13900000001 second")
    assert "<PHONE_001>" in out1 and "<PHONE_001>" in out2  # same placeholder for same value


def test_pii_scrub_address() -> None:
    s = Scrubber()
    out = s.scrub("收货地址: 上海市浦东新区张江路999号")
...
def test_pii_scrub_nick() -> None:
    s = Scrubber()
    out = s.scrub("用户小明留言：东西不错", nicks=["小明"])
...
def test_pii_scrub_no_pii_unchanged() -> None:
    s = Scrubber()
    assert s.scrub("一切正常没有敏感信息") == "一切正常没有敏感信息"
